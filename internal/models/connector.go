...
// Connector represents a transformation component that can use either field mappings or Python scripts
type Connector struct {
	...
	PythonScript   string         `json:"python_script,omitempty" gorm:"type:text"`
	...
	FieldMappings []FieldMapping    `json:"field_mappings,omitempty" gorm:"foreignKey:ConnectorID"`
}
...
// Validate ensures that a connector has either a Python script or field mappings
func (c *Connector) Validate() error {
	if c.PythonScript == "" && len(c.FieldMappings) == 0 {
		return fmt.Errorf("validation failed: connector must have either a python_script or field_mappings defined")
	}
	return nil
}
